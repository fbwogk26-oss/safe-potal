...
- **Tables**:
  ...
  - `risk_assessments`: KRAS risk assessments (period type, department, hazard, frequency(가능성 1-5) × severity(중대성 1-4) = riskScore(max 20), riskLevel A/B/C, beforePhotoUrl for pre-improvement photo; A등급(≥8) items have improvement workflow: improvementMeasures, plannedDate, completionDate, afterFrequency, afterSeverity, afterRiskScore, afterRiskLevel, improvementStatus(미완료/진행중/완료), afterPhotoUrl stored separately via PUT /api/risk-assessments/:id/improvement)
