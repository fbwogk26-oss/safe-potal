  ...
  app.get('/api/safety-cost-records/export', isAuthenticated, async (req: any, res) => {
    try {
      ...
      function buildWorkbook(withImages: boolean): ExcelJS.Workbook {
        ...
        // ════════════════════════════════════════════════════════════
        // Sheet 6: 안전관리비 (CAT2·4·6·7·8 — 전용 시트 없는 항목들)
        // 항목별 → 월별 그룹: 품명/업체/금액 + 견적서·거래명세서·세금계산서
        // ════════════════════════════════════════════════════════════
        const CAT2 = "2. 안전시설비 등";
        const CAT4 = "4. 안전진단비 등";
        const CAT6 = "6. 근로자 건강관리비 등";
        const CAT7 = "7. 건설재해예방 기술지도비";
        const CAT8 = "8. 본사사용비";
        const safetyMgmtCats = [CAT2, CAT4, CAT6, CAT7, CAT8];

        const ws6 = wb.addWorksheet("안전관리비");
        setupCols(ws6);
        let r6 = 1;
        r6 = addTitle(ws6, r6, `안전관리비 증빙자료 (${rangeLabel})`, "FFFFFFFF", "FF375623");

        const safetyMgmtRecs = records.filter(r => safetyMgmtCats.includes(r.category));

        if (safetyMgmtRecs.length === 0) {
          addFullLabel(ws6, r6, "해당 기간에 안전관리비 내역이 없습니다.", "FFFFF0F0");
        } else {
          for (const cat of safetyMgmtCats) {
            const catRecs = safetyMgmtRecs.filter(r => r.category === cat);
            if (catRecs.length === 0) continue;

            const catByYM = groupByYM(catRecs);

            // 항목 구분 헤더
            r6 = addFullLabel(ws6, r6, `▶ ${cat}`, "FFD6E8D4");

            for (const [ym, mRecs] of catByYM) {
              r6 = addMonthHdr(ws6, r6, `  ${ymLabel(ym)} ${cat}`, "FF375623");

              for (const groupRecs of groupByDocPair(mRecs).values()) {
                // ① 공유 문서의 각 항목 정보 표시
                for (const rec of groupRecs) {
                  r6 = addInfoGrid(ws6, r6, [
                    { label: "품명", value: rec.itemName || "-" },
                    { label: "업체", value: rec.vendorName || "-" },
                  ], "FFE8F5EE", "FF375623");
                  r6 = addInfoGrid(ws6, r6, [
                ...
                // ② 첨부 이미지는 한 번만 (대표 레코드 기준)
                const rep6 = groupRecs[0];
                if (rep6.taxInvoiceFileUrl) {
                  r6 = addFullLabel(ws6, r6, "  💰 세금계산서", "FFFFF3D6");
                  r6 = addFullImg(ws6, r6, getPages(rep6.taxInvoiceFileUrl), true);
                }
                const hasQ6 = !!rep6.quoteFileUrl;
                const hasT6 = !!rep6.transactionFileUrl;
                if (hasQ6 || hasT6) {
                  r6 = addLRLabel(ws6, r6, "  📄 견적서", "  📋 거래명세서", "FFD6E4F7", "FFD6F0E4");
                  r6 = addImgPair(ws6, r6, hasQ6 ? getPages(rep6.quoteFileUrl) : [], hasT6 ? getPages(rep6.transactionFileUrl) : [], hasQ6, hasT6);
                }
                if (rep6.certificateFileUrl) {
                  r6 = addFullLabel(ws6, r6, "  🎓 수료증 / 이수증", "FFEAD6F7");
                  r6 = addFullImg(ws6, r6, getPages(rep6.certificateFileUrl), true);
                }
              }

              ws6.getRow(r6).height = 14;
              r6++;
            }
          }
        }
        ...
        for (const txt of infoRows) {
          const row = ws7.getRow(r7);
          row.height = 22;
          const c = row.getCell(1);
          c.value = txt;
          c.font = { bold: false, size: 10 };
          c.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFF2F7FC" } };
          c.alignment = { horizontal: "left", vertical: "middle" };
          c.border = THIN_BORDER;
          ws7.mergeCells(`A${r7}:${T7_LAST}${r7}`);
          r7++;
