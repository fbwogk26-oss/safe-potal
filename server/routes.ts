  ...
  app.post('/api/safety-inspections/bulk-parse', isAuthenticated,
    ...
    async (req: any, res: any) => {
      try {
        ...
        // 엑셀 컬럼 공통 헬퍼 (excelData 기반)
        const buildExcelMatcher = (excelData: Record<string, any>[]) => {
        ...
        // 단일 PDF 처리 함수 (병렬 실행용)
        const processSinglePdf = async (f: any, pdfIndex: number): Promise<any> => {
          try {
            ...
            if (methodLine) { const m = methodLine.match(/점검방법\s*:\s*(.+)/); if (m) inspectionMethod = m[1].trim(); }

            // 팀명 (점검대상)
            const targetLine = lines.find((l: string) => l.includes('점검대상'));
            if (targetLine) {
              const m = targetLine.match(/점검대상\s*:\s*(.+)/);
              if (m) { const parts = m[1].trim().split('>'); team = parts[parts.length - 1].trim(); }
            }
            ...
            for (const pat of inspectorPatterns) {
              const m = fullText.match(pat);
            ...
            for (const pat of workContentPatterns) {
              const m = fullText.match(pat);
            ...
            // 점검결과
            const resultIdx = lines.findIndex((l: string) => /양호|미흡/.test(l) && /\d+/.test(l));
            if (resultIdx >= 0) {
              const rline = lines[resultIdx];
              const mr = rline.match(/양호|미흡/); if (mr) inspectionResult = mr[0];
              const md = rline.match(/(\d+)\s*$/); if (md) defectCount = parseInt(md[1]);
            } else {
              const resLine = lines.find((l: string) => l === '양호' || l === '미흡');
              if (resLine) inspectionResult = resLine;
              const defLine = lines.find((l: string) => /^(미흡건수\s+)?\d+$/.test(l));
              if (defLine) defectCount = parseInt(defLine.replace(/\D/g, '')) || 0;
            }

            // 이미지 추출 — Poppler 우선, 실패 시 JPEG 바이너리 스캔 폴백
            const imageUrls: string[] = [];
            try {
              let imgBuffers = await extractImagesWithPoppler(pdfBuffer);
              if (imgBuffers.length === 0) imgBuffers = extractJpegsFromBuffer(pdfBuffer);
              await Promise.all(
                imgBuffers.slice(0, 10).map(async (buf: Buffer, i: number) => {
                  const filename = `pdf-bulk-${Date.now()}-${pdfIndex}-${i}.jpg`;
                  const objUrl = await uploadToObjectStorage(buf, filename, 'image/jpeg');
                  if (objUrl) imageUrls.push(objUrl);
                  else { fs.writeFileSync(path.join(uploadDir, filename), buf); imageUrls.push(`/uploads/${filename}`); }
                })
              );
            } catch (e) {
              console.error('[bulk-parse] 이미지 추출 오류:', f.originalname, e);
            }
            ...
            if (excelMatcher) {
              ...
              if (teamCol && pdfTeam && pdfDate8 && pdfTime && dateCol) {
                matchedRow = excelData.find((row: Record<string, any>) => {
              ...
              // 4순위: 작업번호 매칭
              if (!matchedRow && workNo && workNoCol) {
                const normalWorkNo = workNo.replace(/\s/g, '').toLowerCase();
                matchedRow = excelData.find((row: Record<string, any>) => {
                  const rowNo = String(row[workNoCol] || '').replace(/\s/g, '').toLowerCase();
                  return rowNo && (rowNo === normalWorkNo || rowNo.includes(normalWorkNo) || normalWorkNo.includes(rowNo));
                });
              }
              // 5순위: 날짜만
              if (!matchedRow && pdfDate8 && dateCol) {
                matchedRow = excelData.find((row: Record<string, any>) => extractDate8(String(row[dateCol] || '')) === pdfDate8);
              }
              // 6순위: 인덱스 순서
        ...
        // 모든 PDF를 병렬로 동시에 처리 (순차 처리 대비 수십 배 빠름)
        console.log(`[bulk-parse] ${pdfFiles.length}개 PDF 병렬 처리 시작...`);
        const results = await Promise.all(pdfFiles.map((f: any, i: number) => processSinglePdf(f, i)));
        console.log(`[bulk-parse] ${pdfFiles.length}개 PDF 처리 완료`);
  ...
  app.get('/api/weather/current-heat', isAuthenticated, async (req, res) => {
    ...
    const ALL_REGION_SETS: Record<string, { name: string; nx: number; ny: number }[]> = {
      daegubuk: [
        ...
        { name:'포항시', nx:102, ny:94  }, { name:'경주시', nx:100, ny:91  },
        { name:'김천시', nx:80,  ny:96  }, { name:'안동시', nx:91,  ny:106 },
        { name:'구미시', nx:84,  ny:96  }, { name:'영주시', nx:89,  ny:111 },
        { name:'영천시', nx:95,  ny:93  }, { name:'상주시', nx:81,  ny:102 },
        { name:'문경시', nx:81,  ny:106 }, { name:'경산시', nx:91,  ny:90  },
        { name:'의성군', nx:88,  ny:101 }, { name:'청송군', nx:96,  ny:103 },
        { name:'영양군', nx:97,  ny:108 }, { name:'영덕군', nx:102, ny:103 },
        { name:'청도군', nx:91,  ny:86  }, { name:'고령군', nx:83,  ny:87  },
        { name:'성주군', nx:83,  ny:91  }, { name:'칠곡군', nx:85,  ny:93  },
        { name:'예천군', nx:84,  ny:107 }, { name:'봉화군', nx:90,  ny:115 },
        { name:'울진군', nx:102, ny:112 }, { name:'울릉군', nx:127, ny:127 },
      ],
      chungcheong: [
        { name:'대전',   nx:67, ny:100 }, { name:'세종',   nx:66, ny:103 },
        { name:'청주시', nx:69, ny:107 }, { name:'충주시', nx:76, ny:114 },
        { name:'제천시', nx:81, ny:118 }, { name:'보은군', nx:74, ny:105 },
        { name:'옥천군', nx:71, ny:99  }, { name:'영동군', nx:74, ny:97  },
        { name:'증평군', nx:71, ny:110 }, { name:'진천군', nx:68, ny:111 },
        { name:'괴산군', nx:76, ny:110 }, { name:'음성군', nx:73, ny:114 },
        { name:'단양군', nx:84, ny:115 },
        { name:'천안시', nx:63, ny:110 }, { name:'공주시', nx:63, ny:102 },
        { name:'보령시', nx:54, ny:100 }, { name:'아산시', nx:60, ny:110 },
        { name:'서산시', nx:51, ny:110 }, { name:'논산시', nx:62, ny:97  },
        { name:'계룡시', nx:65, ny:100 }, { name:'당진시', nx:54, ny:112 },
        { name:'금산군', nx:69, ny:95  }, { name:'부여군', nx:59, ny:99  },
        { name:'서천군', nx:56, ny:94  }, { name:'청양군', nx:59, ny:103 },
        { name:'홍성군', nx:55, ny:106 }, { name:'예산군', nx:58, ny:108 },
        { name:'태안군', nx:48, ny:108 },
      ],
      honam: [
        { name:'광주',   nx:58, ny:74  },
        { name:'전주시', nx:63, ny:89  }, { name:'군산시', nx:56, ny:92  },
        { name:'익산시', nx:60, ny:91  }, { name:'정읍시', nx:58, ny:83  },
        { name:'남원시', nx:68, ny:80  }, { name:'김제시', nx:59, ny:88  },
        { name:'완주군', nx:63, ny:90  }, { name:'진안군', nx:68, ny:88  },
        { name:'무주군', nx:72, ny:90  }, { name:'장수군', nx:70, ny:85  },
        { name:'임실군', nx:66, ny:83  }, { name:'순창군', nx:63, ny:79  },
        { name:'고창군', nx:56, ny:81  }, { name:'부안군', nx:55, ny:87  },
        { name:'목포시', nx:50, ny:67  }, { name:'여수시', nx:73, ny:66  },
        { name:'순천시', nx:70, ny:70  }, { name:'나주시', nx:56, ny:71  },
        { name:'광양시', nx:74, ny:67  }, { name:'담양군', nx:61, ny:78  },
        { name:'곡성군', nx:65, ny:74  }, { name:'구례군', nx:69, ny:72  },
        { name:'고흥군', nx:66, ny:62  }, { name:'보성군', nx:62, ny:66  },
        { name:'화순군', nx:61, ny:72  }, { name:'장흥군', nx:59, ny:63  },
        { name:'강진군', nx:57, ny:60  }, { name:'해남군', nx:54, ny:56  },
        { name:'영암군', nx:55, ny:65  }, { name:'무안군', nx:52, ny:68  },
        { name:'함평군', nx:54, ny:72  }, { name:'영광군', nx:52, ny:76  },
        { name:'장성군', nx:57, ny:77  }, { name:'완도군', nx:57, ny:52  },
        { name:'진도군', nx:48, ny:55  }, { name:'신안군', nx:45, ny:63  },
        { name:'제주시',   nx:53, ny:38  }, { name:'서귀포시', nx:52, ny:33 },
      ],
      buulgyeong: [
        { name:'부산',   nx:98,  ny:76  }, { name:'울산',   nx:102, ny:84  },
        { name:'창원시', nx:90,  ny:77  }, { name:'진주시', nx:81,  ny:75  },
        { name:'통영시', nx:87,  ny:68  }, { name:'사천시', nx:82,  ny:71  },
        { name:'김해시', nx:95,  ny:77  }, { name:'밀양시', nx:96,  ny:86  },
        { name:'거제시', nx:90,  ny:68  }, { name:'양산시', nx:97,  ny:80  },
        { name:'의령군', nx:87,  ny:77  }, { name:'함안군', nx:89,  ny:78  },
        { name:'창녕군', nx:92,  ny:84  }, { name:'고성군', nx:85,  ny:71  },
        { name:'남해군', nx:80,  ny:68  }, { name:'하동군', nx:76,  ny:72  },
        { name:'산청군', nx:80,  ny:78  }, { name:'함양군', nx:77,  ny:83  },
        { name:'거창군', nx:80,  ny:87  }, { name:'합천군', nx:85,  ny:84  },
      ],
    };
